...
\item View after request: `sampling\_stations.csv'  \\~\\
...
Values in these columns describe whether the data passed automated QAQC checks: \href{http://cdmo.baruch.sc.edu/data/qaqc.cfm}{cdmo.baruch.sc.edu/data/qaqc.cfm} \\~\\
...
%%%%%%
\begin{frame}{Format and potential issues with output data}
The final piece of the puzzle is the DateTimeStamp \\~\\
...
%%%%%%
\begin{frame}[fragile]{Overview of the SWMPr package}
Use the following to import some data into R... these files are included with the package, otherwise specify the path manually with the commented line for dataset1 \\~\\
<<eval = F, results = 'markup'>>=
