...
<<>>=
# combine weather and nutrient
tmp <- comb(met_dat, nut_dat, timestep = 30)
...
# check combined data
sum_fun <- function(x) sum(!is.na(x))
apply(tmp, 2, sum_fun) # apply the function to each column in tmp

# check original nutrient data
apply(nut_dat, 2, sum_fun)
@
