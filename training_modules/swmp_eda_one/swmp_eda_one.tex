...
\item operator: value symbol if only one date in suset
...
\hlstd{tmp} \hlkwb{<-} \hlkwd{subset}\hlstd{(wq_dat)}
...
\end{kframe}
\end{knitrout}
Not an issue, further processing may create empty rows/columns
\end{frame}
...
%%%%%%
\begin{frame}[containsverbatim]{Organize SWMP data}
A simple application of `subset' - plot dissolved oxygen and water temperature for October 2014
\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
What are some challenges that we need to consider for combining data? \\~\\
\end{frame}
