...
%%%%%%
\begin{frame}[containsverbatim]{Organize SWMP data}
The `select' argument of `subset.swmpr' is used to select parameters of interested - one to many
\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\item method: how are the data combined - union, intersect, or using a station
\end{itemize}
\end{frame}
...
\end{kframe}
\end{knitrout}
Serious data reduction! Standardizing nutrient data will take some trial and error
\end{frame}

%%%%%%
\begin{frame}[containsverbatim]{Organize SWMP data}
Standardizing nutrient data is easier in combination with other data
\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# combine weather and nutrient}
\hlstd{tmp} \hlkwb{<-} \hlkwd{comb}\hlstd{(met_dat, nut_dat,} \hlkwc{timestep} \hlstd{=} \hlnum{30}\hlstd{)}

\hlkwd{head}\hlstd{(tmp)}
\end{alltt}
\begin{verbatim}
##         datetimestamp po4f nh4f no2f no3f no23f chla_n atemp rh   bp wspd
## 1 2011-01-01 00:00:00   NA 0.04   NA   NA    NA      4    15 94 1019    3
## 2 2011-01-01 00:30:00   NA 0.04   NA   NA    NA      4    15 95 1019    3
## 3 2011-01-01 01:00:00   NA 0.04   NA   NA    NA      4    15 95 1018    3
## 4 2011-01-01 01:30:00   NA 0.04   NA   NA    NA      4    16 94 1018    4
## 5 2011-01-01 02:00:00   NA 0.04   NA   NA    NA      4    16 93 1018    5
## 6 2011-01-01 02:30:00   NA 0.04   NA   NA    NA      4    15 94 1018    5
##   maxwspd wdir sdwdir totpar totprcp cumprcp totsorad
## 1       3  145      8     NA       0       0       NA
## 2       4  139      7     NA       0       0       NA
## 3       4  144      6     NA       0       0       NA
## 4       6  137      7     NA       0       0       NA
...
%%%%%%
\begin{frame}[containsverbatim]{Organize SWMP data}
Did we lose any of the nutrient data?
\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# how many non-missing values in eah column?}

\hlcom{# check combined data}
\hlstd{sum_fun} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)} \hlkwd{sum}\hlstd{(}\hlopt{!}\hlkwd{is.na}\hlstd{(x))}
\hlkwd{apply}\hlstd{(tmp,} \hlnum{2}\hlstd{, sum_fun)} \hlcom{# apply the function to each column in tmp}
\end{alltt}
\begin{verbatim}
## datetimestamp          po4f          nh4f          no2f          no3f 
##         66775         15555         16072             0             0 
##         no23f        chla_n         atemp            rh            bp 
##         15569         16075         60856         60785         66441 
##          wspd       maxwspd          wdir        sdwdir        totpar 
##         62357         62357         62357         62357         44803 
##       totprcp       cumprcp      totsorad 
##         62155         62016             0
\end{verbatim}
\begin{alltt}
\hlcom{# check original nutrient data}
\hlkwd{apply}\hlstd{(nut_dat,} \hlnum{2}\hlstd{, sum_fun)}
\end{alltt}
\begin{verbatim}
## datetimestamp          po4f          nh4f          no2f          no3f 
##            48            24            40             0             0 
...
\end{kframe}
\end{knitrout}
Some data were lost but not much
\end{frame}
